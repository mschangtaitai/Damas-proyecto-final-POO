...
public class CheckersMain extends Application {//Se utiliza para crear la aplicación en javaFX
    ...
    public static final int TILE_SIZE = 80;//TAMAÑO DE LA CASILLA
    ...
    public CheckersMain() throws IOException, ParseException {
    ...
    private Parent createContent() {
        ...
        for (int y = 0; y < HEIGHT; y++) {
            for (int x = 0; x < WIDTH; x++) {
                ...
                if (y <= 2 && (x + y) % 2 != 0) {
                    piece = makePiece(PieceType.ROJA, x, y);
                }

                if (y >= 5 && (x + y) % 2 != 0) {
                    piece = makePiece(PieceType.AZUL, x, y);
                }
    ...
    private void sendMsg(int x0, int y0, int newX, int newY, Piece piece) {
        JSONObject msg = new JSONObject();
        msg.put("x0",x0+1);
        msg.put("y0",y0+1);
        msg.put("newX",newX+1);
        msg.put("newY",newY+1);
        msg.put("piece",piece.getType());
        System.out.println("Writing to socket");
        socketOut.println(msg.toJSONString());
    }
        ...
        piece.setOnMouseReleased(e -> {
            ...
            switch (result.getType()) {
                ...
                case KILL:
                    ...
                    if(piece.getType() == PieceType.ROJA){
                        redCounterNum ++;
                    ...
                    if (redCounterNum == 12|| blueCounterNum ==12) {
                        Platform.exit();
                        System.exit(0);
                    }
                    sendMsg(x0,y0,newX,newY,piece);
